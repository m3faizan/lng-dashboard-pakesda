  ...
  useEffect(() => {
    const fetchKPIData = async () => {
      try {
        const {
          data: lngInfo
        } = await supabase.from('LNG Information').select('date').order('date', {
          ascending: false
        }).limit(1);
        if (lngInfo && lngInfo[0]?.date) {
          const dateObj = new Date(lngInfo[0].date);
          setLatestDate(dateObj.toLocaleString('default', {
            month: 'long',
            year: 'numeric'
          }));
        }
        const {
          data: lngInfoData
        } = await supabase.from('LNG Information').select('date, import_Volume, Total_Cargoes').order('date', {
          ascending: false
        }).limit(2);
        const {
          data: priceInfo
        } = await supabase.from('LNG Port_Price_Import').select('date, wAvg_DES').order('date', {
          ascending: false
        }).limit(2);
        const {
          data: powerInfo
        } = await supabase.from('LNG Power Gen').select('date, rlngShare').order('date', {
          ascending: false
        }).limit(2);
        if (lngInfoData && priceInfo && powerInfo) {
          ...
          setKpiData({
            imports: {
              ...
              trend: calculateTrend(lngInfoData[0]?.import_Volume || 0, lngInfoData[1]?.import_Volume || 0)
            },
            cargoes: {
              value: lngInfoData[0]?.Total_Cargoes || 0,
              trend: calculateTrend(lngInfoData[0]?.Total_Cargoes || 0, lngInfoData[1]?.Total_Cargoes || 0)
            },
            price: {
              value: priceInfo[0]?.wAvg_DES || 0,
              trend: calculateTrend(priceInfo[0]?.wAvg_DES || 0, priceInfo[1]?.wAvg_DES || 0)
            },
            share: {
              value: powerInfo[0]?.rlngShare || 0,
              trend: calculateTrend(powerInfo[0]?.rlngShare || 0, powerInfo[1]?.rlngShare || 0)
            }
          });
        }
      } catch (error) {
        ...
        <main className="flex-1 p-8 overflow-auto">
          <div className="space-y-8 animate-fade-in max-w-[1400px] mx-auto">
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
              ...
              <KPICard title="Total Cargoes" value={kpiData?.cargoes.value.toString() || "0"} icon={<BarChart3 className="h-4 w-4 text-dashboard-green" />} trend={{
              value: Math.abs(kpiData?.cargoes.trend || 0),
              isPositive: (kpiData?.cargoes.trend || 0) > 0
            }} type="cargoes" />
              <KPICard title="Avg Contract Price" value={`$${kpiData?.price.value.toFixed(1)}/MMBtu`} icon={<DollarSign className="h-4 w-4 text-dashboard-green" />} trend={{
              value: Math.abs(kpiData?.price.trend || 0),
              isPositive: (kpiData?.price.trend || 0) > 0
            }} type="price" />
              <KPICard title="Power Gen Share" value={`${kpiData?.share.value.toFixed(1)}%`} icon={<Zap className="h-4 w-4 text-dashboard-green" />} trend={{
            ...
            <div className="space-y-6">
              <div className="p-6 bg-dashboard-navy rounded-lg border-0">
                <h2 className="text-xl font-semibold mb-4 text-center">LNG Import Volumes</h2>
                <LNGChart />
              </div>
