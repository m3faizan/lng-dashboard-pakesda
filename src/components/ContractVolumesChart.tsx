...
export function ContractVolumesChart() {
  ...
  const CustomLegend = () => {
    const items = [
      { name: "Long Term", color: "#4ADE80" },
      { name: "Spot", color: "#0EA5E9" },
      { name: "Total", color: "#FEF7CD" }
    ];
      ...
      <div className="flex justify-center items-center gap-4 mt-2">
        {items.map((item) => (
          ...
          >
            ...
            />
            <span 
              className={`text-xs ${hiddenSeries.includes(item.name) ? 'text-gray-500' : 'text-gray-300'}`}
            >
              {item.name}
            </span>
          </div>
  ...
  return (
    <Card className="bg-dashboard-navy border-0 h-[400px] transition-all duration-300 hover:ring-2 hover:ring-dashboard-blue/20 hover:shadow-lg">
      ...
      <CardContent className="flex flex-col justify-between h-[330px]">
        ...
        {/* Custom legend in a separate div with proper spacing */}
        <CustomLegend />
      </CardContent>
    </Card>
  );
