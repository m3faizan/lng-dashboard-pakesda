...
export function ImportPaymentsChart() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // For mobile, limit the data points to improve readability
      const filteredData = isMobile && selectedPeriod === "monthly" 
        ? processedData.filter((_, index) => index % 2 === 0 || index === processedData.length - 1)
        : processedData;
