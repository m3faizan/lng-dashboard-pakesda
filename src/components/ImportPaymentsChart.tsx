...
export function ImportPaymentsChart() {
  ...
  return <Card className="bg-dashboard-navy border-0 h-[500px] md:h-[480px] w-full transition-all duration-300 hover:ring-2 hover:ring-dashboard-blue/20 hover:shadow-lg">
      ...
      <CardContent className="p-6 bg-dashboard-navy rounded-lg border-0 py-[36px]">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={data} margin={chartMargin}>
            <CartesianGrid strokeDasharray="3 3" stroke="#374151" opacity={0.5} />
            <XAxis dataKey="period" stroke="#94a3b8" tick={{
            fill: "#94a3b8"
          }} fontSize={isMobile ? 11 : 12} height={60} angle={-45} textAnchor="end" interval={isMobile ? "preserveStartEnd" : 0} tickMargin={25} tickSize={8} />
            <YAxis yAxisId="left" stroke="#94a3b8" tick={{
            fill: "#94a3b8"
          }} tickFormatter={formatImportPayment} width={isMobile ? 60 : 65} fontSize={isMobile ? 11 : 12} tickMargin={8} tickCount={6} domain={[0, 'auto']} />
            <YAxis yAxisId="right" orientation="right" stroke="#94a3b8" tick={{
            fill: "#94a3b8"
          }} width={isMobile ? 55 : 60} fontSize={isMobile ? 11 : 12} tickMargin={8} tickCount={6} domain={[0, 'auto']} />
            ...
            gap: "1.5rem"
          }} iconSize={isMobile ? 16 : 14} iconType="circle" />
            <Bar yAxisId="left" dataKey="importPayment" name="Import Payments" fill="#4ADE80" hide={hiddenSeries.includes("importPayment")} radius={[4, 4, 0, 0]} maxBarSize={isMobile ? 40 : 50} />
            <Line yAxisId="right" type="monotone" dataKey="brentAvg" name="Avg. Brent Price" stroke="#FEF7CD" strokeWidth={isMobile ? 2.5 : 2} dot={false} hide={hiddenSeries.includes("brentAvg")} />
          </ComposedChart>
        </ResponsiveContainer>
      </CardContent>
    </Card>;
