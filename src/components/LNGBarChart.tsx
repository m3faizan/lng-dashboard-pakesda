...
export function LNGBarChart() {
  ...
  return <Card className="bg-dashboard-navy border-0 h-[350px] md:h-[480px] w-full transition-all duration-300 hover:ring-2 hover:ring-dashboard-blue/20 hover:shadow-lg overflow-hidden">
      <div className="flex flex-col items-center pt-4 md:pt-6">
        <CardTitle className="text-lg font-semibold text-center mb-2 md:mb-4 md:text-lg">
          LNG Price
        </CardTitle>
      ...
      <CardContent className="h-[250px] md:h-[400px] px-2 md:px-4">
        <ResponsiveContainer width="100%" height="75%">
          <LineChart data={data} margin={chartMargin}>
            ...
            <XAxis dataKey="period" stroke="#525252" fontSize={isMobile ? 10 : 12} tickLine={false} axisLine={false} angle={-45} textAnchor="end" height={60} interval={isMobile ? 1 : "preserveStartEnd"} />
            <YAxis stroke="#525252" fontSize={isMobile ? 10 : 12} tickLine={false} axisLine={false} width={isMobile ? 40 : 60} label={{
            value: getYAxisLabel(),
            angle: -90,
            position: 'insideLeft',
            style: {
        ...
        <div className="flex flex-wrap justify-center gap-1 md:gap-2 mt-4 md:mt-8 mb-4 md:mb-8">
          {timeframes.map(tf => <button key={tf.label} onClick={() => setSelectedTimeframe(tf.months)} className={`px-2 md:px-3 py-1 rounded-md text-xs md:text-sm ${selectedTimeframe === tf.months ? "bg-dashboard-blue text-white" : "bg-dashboard-dark/50 text-muted-foreground hover:bg-dashboard-dark"}`}>
              {tf.label}
            </button>)}
        </div>
      </CardContent>
