...
export function LNGChart() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const {
        data: lngData,
        error
      } = await supabase.from('LNG Information').select('date, import_Volume').gte('date', startDate.toISOString()).lte('date', latestDate.toISOString()).order('date', {
        ascending: true
      });
      if (error) {
      ...
      const transformedData = lngData.filter(item => item.date && item.import_Volume !== null).map((item, index, array) => {
  ...
  return (
    <div className="space-y-4 md:space-y-6 mx-3 md:mx-[19px]">
      <div className="flex flex-col items-center space-y-4">
        <h2 className="text-lg md:text-xl font-semibold">LNG Import Volume</h2>
        ...
        <div className="flex flex-wrap justify-center gap-2 px-2">
          {timeframes.map((tf) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-md text-sm font-medium min-w-[48px] ${
        ...
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data}>
            ...
            <YAxis
              ...
              tickLine={false}
              axisLine={false}
              tickFormatter={value => showPercentage ? `${value.toFixed(1)}%` : `${(value / 1000000).toFixed(1)}M`}
